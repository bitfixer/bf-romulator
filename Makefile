...
fetch_roms: $(BIN_DIR) $(TOOLS_DIR)/fetch_roms.py $(MEMORY_SET)
	cd $(BIN_DIR); python ../$(TOOLS_DIR)/fetch_roms.py $(MEMORY_SET) $(BASEURL)


# FPGA

$(BIN_DIR)/memorymap.bin: $(BIN_DIR) $(MEMORY_SET) $(BIN_DIR)/build_memory_map_set
	cd $(BIN_DIR); ./build_memory_map_set < $(MEMORY_SET) > memorymap.bin

$(BIN_DIR)/enable_table.txt: $(BIN_DIR) $(BIN_DIR)/build_enable_table $(ENABLE_TABLE)
